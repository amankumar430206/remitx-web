      ...
      <ContentCard>
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            {rates.length > 0 && (
              <p className="text-xs text-muted-fg mt-0.5">{rates.length} pairs · refreshes every minute · click to fill calculator</p>
            )}
          </div>
          {provider === 'zoqq' ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {rates.map(rate => {
            const spread = ((parseFloat(rate.clientRate) - parseFloat(rate.midRate)) / parseFloat(rate.midRate) * 100).toFixed(2)
            const flag = (code: string) => CURRENCY_FLAG[code] ?? '🌐'
            return (
              <button
                key={`${rate.from}-${rate.to}`}
                type="button"
                onClick={() => { setCalcFrom(rate.from); setCalcTo(rate.to) }}
                className="group flex flex-col gap-3 rounded-xl border border-border bg-surface p-4 text-left hover:border-primary/40 hover:bg-primary/[0.02] transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-primary/30"
              >
                {/* Currency pair */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    <span className="text-base leading-none">{flag(rate.from)}</span>
                    <span className="text-xs font-bold text-foreground tracking-wider">{rate.from}</span>
                    <svg className="h-3 w-3 text-muted-fg/40 mx-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3" />
                    </svg>
                    <span className="text-base leading-none">{flag(rate.to)}</span>
                    <span className="text-xs font-bold text-foreground tracking-wider">{rate.to}</span>
                  </div>
                  <svg className="h-3 w-3 text-muted-fg/0 group-hover:text-primary/50 transition-colors shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-fg mt-1">per 1 {rate.from}</p>
                </div>

                {/* Mid + spread footer */}
                <div className="flex items-center border-t border-border pt-3 -mx-1">
                  <div className="flex-1 px-1">
                    <p className="text-[10px] font-semibold text-muted-fg/60 uppercase tracking-widest mb-0.5">Mid rate</p>
                    <p className="text-xs font-mono text-muted-fg tabular-nums">{parseFloat(rate.midRate).toFixed(4)}</p>
                  </div>
                  <div className="w-px h-7 bg-border" />
                  <div className="flex-1 px-3">
                    <p className="text-[10px] font-semibold text-muted-fg/60 uppercase tracking-widest mb-0.5">Spread</p>
                    <p className="text-xs font-mono text-muted-fg tabular-nums">{spread}%</p>
                  </div>
                </div>
              </button>
            )
        ...
        {rates.length === 0 && (
          <p className="py-8 text-center text-sm text-muted-fg">No rates available.</p>
        )}
      </ContentCard>
    </div>
  )
