...
const companySchema = z.object({
  ...
  slug: z.string().min(3, 'Minimum 3 characters').max(64).regex(/^[a-z0-9-]+$/, 'Only lowercase letters, numbers and hyphens'),
})

function StepCompany({ onNext }: { onNext: (data: Pick<WizardData, 'companyName' | 'slug'>) => void }) {
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm<z.infer<typeof companySchema>>({
...
function StepPersonal({
  mode, defaultEmail, onNext, onBack,
}: {
  mode: Mode
  defaultEmail?: string
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<z.infer<typeof personalSchema>>({
    resolver: zodResolver(personalSchema),
    defaultValues: { email: defaultEmail ?? '' },
  })

  return (
    <form onSubmit={handleSubmit(d => onNext({ ...d, email: d.email ?? '', phone: d.phone ?? '' }))} className="flex flex-col gap-5">
      ...
      {mode === 'public' && (
        <FormField label="Work email" error={errors.email?.message} htmlFor="email">
          <Input id="email" type="email" placeholder="jane@acmecorp.com" {...register('email')} error={!!errors.email} />
        </FormField>
      )}
...
function StepPassword({
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<z.infer<typeof passwordSchema>>({
  ...
  return (
    <form onSubmit={handleSubmit(d => onNext(d.password))} className="flex flex-col gap-5">
            ...
            <div className="flex gap-1 mb-1">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className={cn(
                  ...
                  strength >= i + 1 ? meta.color : 'bg-border'
                )} />
              ))}
            </div>
            {strength > 0 && (
              <p className={cn('text-xs font-medium', strength <= 2 ? 'text-danger-fg' : strength <= 3 ? 'text-warning' : 'text-success-fg')}>
                {meta.label}
              </p>
            )}
          </div>
      ...
      <FormField label="Confirm password" error={errors.confirm?.message} htmlFor="cpw">
        <Input id="cpw" type={show ? 'text' : 'password'} placeholder="••••••••••••" {...register('confirm')} error={!!errors.confirm} />
      </FormField>
...
type DocState = { file: File | null; status: 'idle' | 'uploading' | 'done' | 'error'; progress: number; error?: string }
...
function DocZone({
  label, hint, accept = '.jpg,.jpeg,.png,.pdf', state, onFile, onRemove,
}: {
  label: string; hint: string; accept?: string
  state: DocState; onFile: (f: File) => void; onRemove: () => void
}) {
  if (state.file && state.status !== 'idle') {
    return (
      <div className={cn(
        'rounded-xl border-2 p-4 flex items-center gap-3 transition-all',
        state.status === 'done'  ? 'border-success-fg/40 bg-success-fg/5' :
        state.status === 'error' ? 'border-danger-fg/40 bg-danger/5' :
  ...
  return (
    <label className="group flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-border hover:border-primary/50 hover:bg-primary/5 p-6 cursor-pointer transition-all">
      <input type="file" className="sr-only" accept={accept} onChange={e => e.target.files?.[0] && onFile(e.target.files[0])} />
...
function StepDocuments({ onNext, onSkip }: { onNext: () => void; onSkip: () => void }) {
  ...
  const handleIdFile = async (file: File) => {
    ...
    if (!initiated) {
      try { await kycApi.initiate(); setInitiated(true) } catch { /* already exists */ setInitiated(true) }
    }
  ...
  const handleAddrFile = async (file: File) => {
    ...
    if (!initiated) {
      try { await kycApi.initiate(); setInitiated(true) } catch { setInitiated(true) }
    }
      ...
      <div className="w-full flex flex-col gap-2">
        {NEXT_STEPS.map(s => (
          ...
          >
            ...
            <svg className="w-4 h-4 text-muted-fg group-hover:text-primary transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
          </Link>
        ))}
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center bg-surface p-8 lg:p-12 overflow-y-auto">
        <div className="w-full max-w-[400px]">
          ...
          {/* Step content */}
          {currentKey === 'company' && (
            <StepCompany onNext={handleCompany} />
          )}
          {currentKey === 'personal' && (
            <StepPersonal
              mode={mode}
          ...
          {currentKey === 'documents' && (
            <StepDocuments onNext={handleDocumentsDone} onSkip={handleDocumentsSkip} />
          )}
          {currentKey === 'complete' && (
            <StepComplete name={wizardData.firstName ?? 'there'} />
          )}
