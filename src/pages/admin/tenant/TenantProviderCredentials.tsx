...
export function TenantProviderCredentials({ tenantId }: Props) {
  ...
  // Tracks which fields just had their value copied (for ✓ flash)
                      ...
                      <div className="relative flex items-center">
                        {isSensitive && !isRevealed ? (
                          ...
                          <Input
                            ...
                            className={cn('font-mono text-sm', isSensitive && isRevealed ? 'pr-16' : 'pr-9')}
                          />
                        )}

                        {/* Copy button — only when revealed and has value */}
                        {field.sensitive && isRevealed && fieldValues[field.key] && (
                          <button
                            type="button"
                            onClick={() => {
                              navigator.clipboard.writeText(fieldValues[field.key])
                              setCopiedFields(prev => new Set([...prev, field.key]))
                              setTimeout(() => setCopiedFields(prev => {
                                const s = new Set(prev); s.delete(field.key); return s
                              }), 2000)
                            }}
                            title="Copy to clipboard"
                            className={cn(
                              'absolute right-9 transition-colors',
                              copiedFields.has(field.key) ? 'text-success-fg' : 'text-muted-fg hover:text-foreground',
                              'focus-visible:outline-none',
                            )}
                          >
                            {copiedFields.has(field.key) ? <CheckIcon /> : <CopyIcon />}
                          </button>
                        )}
